 ...
 */
async function fetchPublicData(base, ua, username, lookbackDays, since, until) {
  ...
  const untilT = until ? Date.parse(until) : null;
  for (const ev of events) {
    if (!contributionTypes.has(ev.type)) continue;
    const t = Date.parse(ev.created_at);
    if (sinceT && t < sinceT) continue;
    if (untilT && t > untilT) continue;
    dates.push(ev.created_at);
  }
